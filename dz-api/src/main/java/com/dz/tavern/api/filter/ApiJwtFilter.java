package com.dz.tavern.api.filter;
...
public class ApiJwtFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        ...
        return path.equals("/api/auth/login")
                || path.equals("/api/auth/authorize")
                || path.equals("/api/store/list")
                || path.equals("/api/pay/notify/wechat")
                || path.equals("/api/pay/notify/refund")
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            log.warn("用户令牌解析失败 path={} remoteAddr={}",
                    request.getRequestURI(), request.getRemoteAddr(), exception);
            writeUnauthorized(response);
            return;
        }
        if (!"USER".equals(principal.type())) {
            writeUnauthorized(response);
            return;
        }
